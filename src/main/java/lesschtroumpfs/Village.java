...
public class Village {
	...
	public void solstice_d_ete() {
		System.out.println("Ici au village "+getNom()+" c'est le jour du solstice d'été\n"
				+ "Chaque Schtroumpf fête ce jour !");
		...
		System.out.println("Il y a en ce jour de solstice d'été "+nbContent+" Schtroumpfs heureux");
	}
	...
	public void modifierSalsePareille(int pfNbSalsePareille) throws Exception{
		if(this.stockSalsepareille + pfNbSalsePareille > 0) {
			...
			throw new Exception("Erreur : Le nombre de Salsepareille est négatif.");
		}
	}
	...
	public void dinnerTousEnsemble() {
		for (int i=0; i<this.getNbSchtroumpfs();i++) {
			try {
				...
				System.out.println("Mince ! Le village est à court de Salsepareille...\n");
				break;
			}
		}
	}
